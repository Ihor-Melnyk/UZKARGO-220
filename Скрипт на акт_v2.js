...
function ProcessActTask() {
  ...
  if ((stateTask == "assigned" || stateTask == "inProgress" || stateTask == "delegated") && !EdocsApi.getAttributeValue("VisaHolders").value) {
    ...
    if (StructureDepart) {
      var data = EdocsApi.findElementByProperty("value", StructureDepart, EdocsApi.getDictionaryData("Commission"))?.code; //беремо значення із довідника "StructureDepart" та шукаємо значення в довіднику "Commission"
